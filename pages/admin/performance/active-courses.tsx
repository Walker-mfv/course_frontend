...
import { useActiveCoursesQuery, useCountActiveCoursesQuery } from 'app/modules/admin/queries/active-courses-query.hook'
      ...
      return {
        ...
        'basicInfo.title': (
          <NextLink href={PathHelper.getCourseDetailPath(item.basicInfo.slug)}>
            <AdminHighlightSearchText value={item.basicInfo.title} fields={['basicInfo.title']} />
          </NextLink>
        ),
        status: <StatusBadge status={item.status as TStatus} />,
        totalSales: <Price value={item.totalSales} />,
        numStudent: item.numStudent,
        'meta.avgRatingScore': <NumberFormat displayType="text" value={item.meta.avgRatingScore} decimalScale={1} />,
