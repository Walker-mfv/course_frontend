...
const Table = () => {
  ...
  const { isLoading: isPendingPaymentLoading, data: pendingPaymentData } = useAcpInstructorPendingPaymentsQuery()
  const {
    state: { page },
  } = useSimplePagination()
  const { isLoading: isPaymentsLoading, data: paymentsData } = useAcpInstructorPaymentsQuery(page, ROWS_PER_PAGE)
  const data = useMemo(() => {
      ...
      >
        ...
        <GridItem colSpan={1}>
          <InfoColumn>
            ...
            <InfoLine label={'Pending amount'} content={<Price value={data?.pendingAmount} />} />
          </InfoColumn>
        </GridItem>
        {data?.pendingAmount ? (
          <GridItem colSpan={1}>
            <InfoColumn>
              <Button onClick={onPayAll} px={10} size="sm" w="fit-content" colorScheme={'blue'}>
                Pay all
              </Button>
            </InfoColumn>
          </GridItem>
