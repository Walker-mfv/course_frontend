...
// REVIEWS
export function apiUpdateReviewResponse(id: string, data: Partial<IReviewResponse>): Promise<boolean> {
...
export function fetchInstructorCourseRatingStats(courseId?: string, viewInstructorId?: string): Promise<IRatingStat[]> {
  const queryString = UrlHelper.cvtObjToQueryString({
    'course._id_filter': courseId,
    viewInstructorId,
  })
