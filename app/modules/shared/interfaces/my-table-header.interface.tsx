...
export const MyTableHeader = ({ columns, checkbox = false, urlParams: clientUrlParams }: MyTableHeaderProps) => {
  ...
  const headerHtml = columns.map(({ header, isNumeric = false, sortable, accessor, minW: w }) => {
    ...
    const newOrder = clientUrlParams._sortBy == accessor ? (clientUrlParams._order == 'asc' ? 'desc' : 'asc') : 'asc'
    let url = ''
    if (sortable) {
    ...
    return (
      <Th minW={w} textAlign={'left'} key={header} isNumeric={isNumeric} color={subtitleColor}>
        {sortable ? (
