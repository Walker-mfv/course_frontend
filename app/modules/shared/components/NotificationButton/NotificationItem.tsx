...
const Time = ({ item }: { item: INotification }) => {
  ...
  return (
    <Text fontSize={'sm'} fontWeight={!item.isRead ? 'bold' : 'normal'} color={!item.isRead ? 'blue.500' : subColor}>
      ...
      <NotificationLink {...props}>
        <MenuItem onClick={onRead} bg={!props.item.isRead ? 'blue.50' : undefined}>
          <HStack
            ...
            pos="relative"
          >
            <Avatar size="md" src={props.item.thumb || AppImg.APP_LOGO} bgColor="white" />
            <Stack spacing={0}>
              <Text as="div" dangerouslySetInnerHTML={{ __html: props.item.content }}></Text>
