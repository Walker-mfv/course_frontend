...
export const useCrudActions = () => {
  ...
  const onDeleteOne = useCallback(
    async (id: string, itemName?: string, options?: IActionOptions): Promise<void> => {
      ...
      const onShowDeleteDialog = () => {
        return new Promise<void>((resolve, reject) => {
          simpleDialog.onShow({
            title: canDelete ? `${lodash.capitalize(lan.DELETE)} ${itemName}` : lodash.capitalize(lan.CANNOT_DELETE),
            content,
            ...
            onPositive: async () => {
              if (canDelete) {
                ...
                router.push(
                  PathHelper.getAdminControllerPage(conflictData?.reference.type, {
  ...
  const onCreate = useCallback(
    async <T>(data: Partial<T>, options?: IActionOptions): Promise<T | null> => {
      ...
      try {
        ...
        queryClient.invalidateQueries([ctrl])
        toast(NotifyHelper.success(`${Helper.lodash.upperFirst(model)} ${Helper.lodash.upperFirst(lan.CREATED)}`))
        return item
      } catch (e) {
        toast(NotifyHelper.somethingWentWrong)
      }
  ...
  const onUpdate = useCallback(
    async <T>(id: string, data: Partial<T>, options?: IActionOptions): Promise<T | null> => {
      ...
      try {
        ...
        queryClient.invalidateQueries([ctrl])
        toast(NotifyHelper.success(`${Helper.lodash.upperFirst(model)} ${Helper.lodash.upperFirst(lan.UPDATED)}`))
        return item
      } catch (e) {
        toast(NotifyHelper.somethingWentWrong)
      }
  ...
  const onXThunkUpdate = useCallback(
    async <T>(asyncThunk: AsyncThunkAction<T, any, {}>, options?: IActionOptions): Promise<T | null> => {
      try {
        await dispatch(asyncThunk)
        queryClient.invalidateQueries([ctrlName])
        toast(NotifyHelper.success(`${Helper.lodash.upperFirst(modelName)} ${Helper.lodash.upperFirst(lan.UPDATED)}`))
      } catch (e) {
        toast(NotifyHelper.somethingWentWrong)
      }
      return null
  ...
  const onDeleteMany = useCallback(
    (ids: string[], options?: IActionOptions) => {
      simpleDialog.onShow({
        title: `${Helper.lodash.capitalize(lan.DELETE)} ${ids.length} ${lan.SELECTED_ITEMS}`,
