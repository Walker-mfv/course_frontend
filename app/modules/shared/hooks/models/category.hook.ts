...
export const useCatPrimarySelectData = (options?: UseQueryOptions<ISelectItem<string>[]>) => {
  return useQuery<ISelectItem<string>[]>(RQK_CATEGORY_PRIMARY_SELECT_DATA, fetchPrimarySelectData, {
...
export const useCatSubSelectData = (parentId: string, options?: UseQueryOptions<ISelectItem<string>[]>) => {
  return useQuery<ISelectItem<string>[]>([RQK_CATEGORY_SUB_SELECT_DATA, parentId], () => fetchSubSelectData(parentId), {
    notifyOnChangeProps: ['data'],
    enabled: !!parentId,
    ...options,
  })
