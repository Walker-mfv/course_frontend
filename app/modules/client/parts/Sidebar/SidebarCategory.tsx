...
const PrimaryCatList = () => {
  ...
  const catItemsHtml = cats.map((item) => {
    return <PrimaryCatItem key={item.slug} slug={item.slug} name={item.name} subCats={item.subCats} />
  })
...
export const SidebarSubCatList = () => {
  ...
  const catItemsHtml = hoveredPrimaryCat?.subCats.map((item, i) => (
    <SubCatItem key={item.slug} primarySlug={hoveredPrimaryCat.slug} name={item.name} slug={item.slug} />
  ))
