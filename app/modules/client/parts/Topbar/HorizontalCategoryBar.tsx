...
const PrimaryCatItem = ({ slug, name, ...props }: ICatInfo & TextProps) => {
  ...
  return (
    <NextLink href={PathHelper.getCategoryPath(slug)}>
      <Text color={slug == hoveredPrimaryCat?.slug ? color : undefined} _hover={{ color }} py={4} {...props}>
        {name}
      </Text>
    </NextLink>
  )
...
const PrimaryCatBar = () => {
  ...
  const catsHtml = cats.map((item, i) => {
    return <PrimaryCatItem onMouseEnter={() => setHoveredPrimaryCat(item)} key={i} name={item.name} slug={item.slug} />
  })
...
const SubCatBar = (props: { primaryCat: ISecondLevelCatInfo }) => {
  const catsHtml = props.primaryCat.subCats.map((item, i) => {
    return <SubCatItem key={i} name={item.name} primarySlug={props.primaryCat.slug} slug={item.slug} />
  })
