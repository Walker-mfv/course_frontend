...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!!userCart) {
      ...
      const merged = Helper.mergeByKey<ICourse>(userCart.courses, localCart?.courses || [], '_id')
      // set cart
  ...
  const onCheckout = useCallback(async () => {
    try {
      ...
      toast(NotifyHelper.warning(AppMsg.TRY_AGAIN_LATER, AppMsg.PAYMENT_SERVICE_NOT_AVAILABLE))
    } finally {
      setPaying(false)
    }
  }, [toast])
