...
import { useAddToWishlist } from '@client/queries/wishlist-query.hook'
...
const CartCoursePrice = ({ course }: { course: ICourse }) => {
  ...
  const prices = CourseHelper.getPrices(course.basicInfo.price, course.basicInfo.currency, course.promotions)
  return (
    <Stack spacing={0}>
...
const CourseActions = ({ course }: { course: ICourse }) => {
  ...
  return (
    <ButtonGroup flexDir={{ base: 'row', sm: 'column' }} alignItems={{ base: 'center', sm: 'end' }}>
        ...
        >
          ...
          <Stack spacing={0}>
            ...
            {course.meta?.avgRatingScore && (
              <Rating size="sm" value={course.meta.avgRatingScore} ratingCount={course.meta.ratingCount} />
            )}
