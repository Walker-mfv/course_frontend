...
export default function CourseCheckout({ size = 'md', course, ...props }: CourseCheckoutProps) {
  let prices = useMemo(() => {
    if (course) return CourseHelper.getPrices(course.basicInfo.price, course.basicInfo.currency, course.promotions)
  }, [course])
  prices = prices!
  return (
    <Stack {...props}>
        ...
        <HStack justify={size == 'sm' ? 'space-between' : 'unset'}>
          ...
          <HStack>
            <Stack spacing={0} flexDir={size == 'md' ? 'row' : 'column'}>
              ...
              {prices.discountPercent > 0 && <Text>{Math.floor(prices.discountPercent * 100)}% off</Text>}
            </Stack>
          </HStack>
        </HStack>
