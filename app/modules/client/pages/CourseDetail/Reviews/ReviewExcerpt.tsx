...
const ReviewContent = ({ review, showAllContent }: { review: IReview; showAllContent: boolean }) => {
  ...
  return (
    <Stack>
      <Text fontSize={'sm'}>
        {isFull || showAllContent ? review.content : review.content?.slice(0, briefLength) + '...'}
      </Text>
  ...
  return (
    <Stack spacing={5}>
      <HStack align={'start'} spacing={{ base: 0, sm: 4 }}>
        <Avatar display={['none', 'inline-block']} name={fullName} src={review.user.profile.avatar || ''} size="md" />
        <Stack flex={1}>
          <HStack justify={'space-between'}>
            <Heading size="sm">{fullName}</Heading>
            <Text color={subColor} fontSize="sm" title={DateHelper.getLongDate(new Date(review.timestamps.createdAt))}>
