...
import { INavGroup, INavItem, INSTRUCTOR_MENU_DATA, useInstructorMenu } from '../../providers/instructor-menu.provider'
    ...
    >
      <HStack>
        <Heading p={4} pl={6} fontSize={'2xl'} color="purple.300" whiteSpace={'nowrap'}>
          O
          <Text as="span" fontSize={'2xl'} opacity={0} className="nav-item-label" transition={'opacity .5s'}>
            nline Courses
          </Text>
        </Heading>
      </HStack>
...
function InstructorSidebar() {
  ...
  return (
    <Box pos="fixed" top={0} left={0} bottom={0} zIndex={'sticky'} w={sidebarWidth + 'px'} bg={bgColor}>
      <PrimaryBar />
