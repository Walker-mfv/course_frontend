...
const Row = ({ item, onPay, onViewDetail }: { item: IPayment } & Actions) => {
  ...
  const button = (
    <Button onClick={() => onViewDetail && onViewDetail(item)} variant={'link'} colorScheme="purple">
      {timePeriod}
    </Button>
  )
  return (
    ...
    >
      <Td>
        {!onViewDetail ? (
          <NextLink href={PathHelper.getInstructorRevenueReportPath(item._id)}>{button}</NextLink>
        ) : (
          button
        )}
      </Td>
...
export default function RevenueReportTable(props: RevenueReportTableProps & Actions) {
  const renderItem = useCallback(
    (item: IPayment, i: number) => {
      return <Row key={item._id + item.status} item={item} onPay={props.onPay} onViewDetail={props.onViewDetail} />
    },
