...
const Main = ({ prevCourseId }: InstructorCourseReviewExcerptProps) => {
  ...
  return (
    <Stack divider={<StackDivider color={borderColor} />} spacing={8}>
      {/* course */}
      {prevCourseId != review?.course?._id && !!review.course ? <ReviewCourseExcerpt item={review.course} /> : null}

      {/* review */}
      <Stack spacing={5}>
        <ReviewExcerpt showResponse={!writingResponse} showAllContent={true} review={review} timeType="long" />
        {!viewMode && (
          <Box pl={[0, 8]}>
            {!writingResponse ? <InstructorCourseReviewExcerptBottomBar /> : <InstructorRespondForm />}
          </Box>
        )}
      </Stack>
    </Stack>
