...
export const useInstructorCourseReviewsQuery = (options?: UseInfiniteQueryOptions<IReview[]>) => {
  ...
  return useInfiniteQuery<IReview[]>(
    ...
    fetchInstructorCourseReviews,
    {
      ...
      getNextPageParam: (_lastPage, pages) => QueryHelper.getNextPageParams(pages.length, 5, count),
      ...options,
    }
  )
}
...
export const useUpdateReviewResponse = () => {
  ...
  return useMutation(
    (payload: { id: string; data: Partial<IReviewResponse> }) => apiUpdateReviewResponse(payload.id, payload.data),
    {
      onMutate: () => {},
      onSuccess: (result) => {
