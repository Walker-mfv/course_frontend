...
const QuestionList = () => {
  ...
  const questionsHtml = quiz?.questions?.map((item, i) => {
    return <QuestionExcerpt key={i} question={item} index={i} />
  })
  return <Stack>{questionsHtml}</Stack>
}
      ...
      {!isEditContent ? (
        <Stack spacing={10}>
          <HStack justify={'space-between'}>
            <Button onClick={onAddQuestion} size="sm" colorScheme={'blue'} leftIcon={<Icon as={AppIcon.add} />}>
              Question
            </Button>
...
const Main = () => {
  ...
  return (
    <>
      {/* HEADER */}
      <CourseUnitHeader title={quiz.title} action={action} />
