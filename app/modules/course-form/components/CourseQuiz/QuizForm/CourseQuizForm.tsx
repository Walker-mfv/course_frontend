...
export default function CourseQuizForm({
  ...
  unitIdx = useAppSelector((state) =>
    formType == 'edit' ? selectSectionNUnitIndex(props.sectionId, props.unitId)(state)[1] : unitIdx!
  )
  const unitNo = useAppSelector((state) =>
    formType == 'edit' ? selectFormCourseUnitNo(props.sectionId, props.unitId)(state) : undefined
  )
  ...
  const onSubmit = handleSubmit((values) => {
    ...
    if (formType == 'edit') {
      ...
      onXThunkUpdate(addCourseUnit({ sectionId: props.sectionId, unitIdx: unitIdx!, unit: data }))
    }
  })
    ...
    <form onSubmit={onSubmit}>
      <CurriculumFormWrapper label={title} border="none">
        <MyInput
          ...
          showLabelRow={false}
        />
        <Editor watch={watch} field="description" setValue={setValue} placeholder="Quiz Description" />
