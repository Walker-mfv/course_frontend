...
export function LectureParamsProvider({ children }: { children: ReactNode }) {
  ...
  const lecture = useSelector(selectFormCourseUnitByIds(address.sectionId, address.unitId))?.lecture as ILecture
  ...
  const addResource = useCallback(
    async (data: { data: Partial<IFile> }) => {
      setProcessingFile(genProcessingFile(data.data.name || '', data.data.type || '', data.data.size || 0, user!._id))
      await dispatch(addCourseLectureResource({ ...data, lectureId: lecture._id, ...address }))
      queryClient.invalidateQueries([RQK_MY_FILES])
      reset()
    },
    [address, dispatch, lecture._id, queryClient, reset, user]
