...
import { apiUpdateLectureVideoFromLibrary } from 'app/apis/course/course-form.api'
import { addCourseLectureResourceId } from 'app/store/course/form-course.slice'
...
const EditableFileName = ({ row, type }: { row: IFile; type: TLibraryTable }) => {
  ...
  const onSubmit = handleSubmit((values) => {
    ...
    onUpdate<IFile>(row._id, { name: fileName }, { ctrlName: 'files', modelName: 'file' }).then((_) => {
  ...
  const onSelectClick = async () => {
    switch (contentType) {
      case 'video':
        ...
        break
      case 'resource':
        const existingItem = (lecture.resources as IFile[]).find((item) => item._id == row._id)
        if (!existingItem) {
          await onXThunkUpdate(addCourseLectureResourceId({ lectureId: lecture._id, resourceId: row._id }))
        } else {
          onShow({
        ...
        {type == 'processing' ? (
          <Tfoot>
            <Tr>
              <Td colSpan={1000}>
                <Text>
                  <Text as="strong">Note:</Text> This video is still being processed.{' '}
                </Text>
              </Td>
            </Tr>
          </Tfoot>
