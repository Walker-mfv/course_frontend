...
function BasicsForm() {
  ...
  const { handleUpload, uploadProgress, getImgSrcFuncRef } = useUploadImage(COURSE_IMAGE_DIR, item?.basicInfo.image)
  const {
    register,
    handleSubmit,
  ...
  useEffect(() => {
    if (!!item) {
      const primaryCat = TypeHelper.isCategory(item?.categories?.[0]) ? item?.categories?.[0] : undefined
      const subCat = TypeHelper.isCategory(item?.categories?.[1]) ? item?.categories?.[1] : undefined
