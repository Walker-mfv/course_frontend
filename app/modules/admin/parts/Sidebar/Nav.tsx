...
function Nav() {
  ...
  const NavItemGroupsHtml = groupItems?.map((item) => {
    ...
    return <NavGroupItem key={item.title} navItems={item.navItems} title={item.title} icon={item.icon} />
  })
  return (
    <>
      <Skeleton isLoaded={!!user} w="full">
        <Accordion w="full" allowMultiple index={indexes} onChange={(item) => setIndexes(item as number[])}>
          {NavItemGroupsHtml}
        </Accordion>
      </Skeleton>
    </>
