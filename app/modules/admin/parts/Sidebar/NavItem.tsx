...
export default function NavItem({ navItem, active = false }: { navItem: IAdminNavItem; active?: boolean }) {
    ...
    <ListItem>
      <NextLink passHref href={navItem.link} linkProps={{ _hover: { textDecor: 'none' } }}>
        <HStack onClick={onClose}>
          ...
          <Text color={!isActive ? subtitleColor : undefined} fontWeight={isActive ? 'bold' : 'normal'}>
            {navItem.title}
          </Text>
        </HStack>
      </NextLink>
