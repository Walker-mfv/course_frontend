...
const Row = ({ item: rolePermission }: { item: IRolePermission }) => {
  ...
  const [enabledPermissionForm, setEnabledPermissionForm] = useState<TPermission[]>(rolePermission.enabledPermissions)
  ...
  const updateRolePermissionDebounce = useCallback(
    lodash.debounce((roleId: string, documentPermissionId: string, enabledPermissions: TPermission[]) => {
  ...
  useEffect(() => {
    if (!!role?._id && !lodash.isEqual(enabledPermissionForm, rolePermission.enabledPermissions)) {
      // update
      updateRolePermissionDebounce(role._id, rolePermission.documentPermission._id, enabledPermissionForm)
    }
  }, [
  ...
  return (
    <Tr>
      ...
      <Td>
        <IconButton onClick={onDelete} size="sm" aria-label="" icon={<Icon as={AppIcon.x} />} />
      </Td>
    </Tr>
  )
}
