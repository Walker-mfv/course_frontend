...
import { useAddRolePermission, useRolePermissionsQuery } from '@admin/queries/role-permissions-query.hook'
...
export default function AddNewRuleForm(props: AddNewRuleFormProps) {
  ...
  const documentPermissionSD = useMemo(() => {
    ...
    const data = documentPermissionData?.filter((item) => !rolePermissionIds?.includes(item.value))
    return data
