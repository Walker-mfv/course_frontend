...
export function useInstructorsWithPaymentQuery(options?: UseQueryOptions<IInstructorWithPayment[]>) {
  const query = useAdminUrlParams()
  return useQuery<IInstructorWithPayment[]>([RQK_INSTRUCTORS_WITH_PAYMENT, query], fetchInstructorsWithPayment, {
...
export function useCountInstructorsWithPayment(query: IClientUrlParams, options?: UseQueryOptions<number>) {
  return useQuery<number>([RQK_INSTRUCTORS_WITH_PAYMENT, query, 'count'], countInstructorsWithPayment, {
    keepPreviousData: true,
    notifyOnChangeProps: 'tracked',
    ...options,
  })
