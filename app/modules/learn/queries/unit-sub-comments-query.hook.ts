...
import { selectActiveUnit, selectLearnCourseId } from 'app/store/course/learn-course.slice'
...
export const useUnitSubCommentsQuery = (
  ...
  return useQuery<IComment[]>([RQK_UNIT_SUB_COMMENTS, courseId, unit?._id, parentId], fetchUnitSubComments, {
    ...options,
...
export const useUpdateUnitSubComment = () => {
  ...
  return useMutation(
    (payload: { id: string; data: Partial<IComment> }) => apiUpdate<IComment>('comments', payload.id, payload.data),
    {
      onMutate: () => {},
      onSuccess: (item) => {
        queryClient.invalidateQueries([RQK_UNIT_SUB_COMMENTS, item.course, item.unit, item.parent])
      },
    }
  )
}
  ...
  return useMutation(
      ...
      }),
    {
      onSuccess: async (comment) => {
        queryClient.invalidateQueries([RQK_UNIT_SUB_COMMENTS, comment.course, comment.unit, comment.parent])
      },
    }
  )
}
