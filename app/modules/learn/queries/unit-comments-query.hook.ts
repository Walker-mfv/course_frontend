...
const useCountUnitComments = (courseId?: string, unitId?: string) => {
  return useQuery<number>([RQK_UNIT_COMMENTS, courseId, unitId, 'count'], () => countUnitComments(courseId!, unitId!), {
...
export const useUnitComments = (unitId?: string, options?: UseInfiniteQueryOptions<IComment[]>) => {
  ...
  return useInfiniteQuery<IComment[]>([RQK_UNIT_COMMENTS, courseId, unitId, itemsPerPage], fetchUnitComments, {
...
export const useUpdateUnitComment = () => {
  const queryClient = useQueryClient()
  return useMutation(
    (payload: { id: string; data: Partial<IComment> }) => apiUpdate<IComment>('comments', payload.id, payload.data),
    {
      onMutate: () => {},
      onSuccess: (item) => {
        queryClient.invalidateQueries([RQK_UNIT_COMMENTS, item.course, item.unit, itemsPerPage])
  ...
  return useMutation(
      ...
      }),
    {
      onMutate: async (newComment) => {
        const keys = [RQK_UNIT_COMMENTS, newComment.course?._id, newComment.unit?._id, itemsPerPage]
        await queryClient.cancelQueries(RQK_UNIT_COMMENTS)
        const previousData = queryClient.getQueryData(keys)
        queryClient.setQueryData<{ pages: IComment[][]; pageParams: number[] }>(keys, (oldQueryData) => {
          return {
            pages: [
              [
                {
      ...
      onSettled: (data) => {
        queryClient.invalidateQueries([RQK_UNIT_COMMENTS, data?.course, data?.unit, itemsPerPage])
      },
    }
  )
}
