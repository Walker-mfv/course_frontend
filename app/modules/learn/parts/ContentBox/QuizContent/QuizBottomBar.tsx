...
const DoingActions = () => {
  ...
  return (
    <>
      <Button onClick={onSkipQuestion} size="sm" variant={'unstyled'} className="no-focus-shadow">
        Skip question
      </Button>
      <Button onClick={onCheckAnswer} isDisabled={typeof selectedOption == 'undefined'} size="sm" colorScheme="purple">
        Check answer
      </Button>
    </>
  )
...
const DoneActions = () => {
  ...
  return (
    <>
      <Button onClick={onNextContent} size="sm" variant={'unstyled'} className="no-focus-shadow">
        Continue
      </Button>
...
export default function QuizBottomBar() {
  ...
  return (
    <HStack justify={'space-between'} py={2} px={8} border="1px solid" borderColor={borderColor} bg={bg}>
      {/* LEFT */}
      <HStack>
        <Text fontSize={['sm', 'md']}>
