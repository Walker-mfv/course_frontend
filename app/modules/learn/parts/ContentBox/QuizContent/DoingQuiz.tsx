...
const AnswerCard = ({ answer }: { answer: IAnswerOption }) => {
  ...
  const isDisabled = lastSelected?.isCorrect ? answer.optionNo != lastSelected.option.optionNo : isSelected
  return (
    <HStack
      border="1px solid"
    ...
    >
      <Radio value={answer.optionNo + ''} spacing={4} p={2} flex={1} isDisabled={isDisabled}>
        <Text as="div" dangerouslySetInnerHTML={{ __html: answer.answerContent || '' }}></Text>
      </Radio>
    </HStack>
  )
}
    ...
    <Stack>
      <Alert status={lastSelected?.isCorrect ? 'success' : 'error'}>
        ...
        <Box>
          ...
          {lastSelected?.option?.description && <AlertDescription>{lastSelected.option.description}</AlertDescription>}
        </Box>
      </Alert>
    </Stack>
  )
  ...
  return (
    <Stack flex={1} spacing={8} py={10}>
      ...
      <Stack>
        ...
        <Text as="div" dangerouslySetInnerHTML={{ __html: question?.questionContent || '' }}></Text>
      </Stack>
      <RadioGroup onChange={(val) => setSelectedOption(Number.parseInt(val))} value={selectedOption + ''}>
