...
const ResultList = (props: { icon?: IconType; iconColor?: TColorScheme; title: string; questions: IQuestion[] }) => {
  const listHtml = props.questions.map((item, i) => <Text key={i}>{Helper.cvtHtmlToText(item.questionContent)}</Text>)
  return (
...
const DoneHeading = (props: { nCorrect: number; nTotal: number }) => {
  ...
  const bg = useColorModeValue(isPassed ? 'green.300' : 'yellow.300', isPassed ? 'green.500' : 'gray.600')
  return (
    ...
    <Stack spacing={0} pb={10}>
      ...
      <QuizContentContainer py={5} spacing={8}>
        ...
        {result.skipQuestions.length > 0 && (
          <ResultList icon={AppIcon.skip} iconColor="black" title="What you skipped" questions={result.skipQuestions} />
        )}
      </QuizContentContainer>
    </Stack>
  )
