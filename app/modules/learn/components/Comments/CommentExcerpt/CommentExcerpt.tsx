...
const CommentContent = ({ comment, parentId }: CommentExcerptProps) => {
  ...
  return (
    <Stack>
      <Text fontSize={'md'}>{isFull ? comment.content : comment.content.slice(0, briefLength) + '...'}</Text>
      {isLong ? (
        !isFull ? (
          <Button onClick={() => setFull(true)} variant={'outline'} colorScheme="purple" size="xs" alignSelf={'start'}>
            Show more
          </Button>
        ) : (
          <Button onClick={() => setFull(false)} variant={'outline'} colorScheme="purple" size="xs" alignSelf={'start'}>
            Show less
          </Button>
        )
      ) : null}
    ...
    <Stack spacing={5}>
      <Stack>
        ...
        {!!props.comment.hasSub && !showSubs ? (
          <HStack>
            <Button size={'sm'} onClick={onViewReplies} colorScheme="purple" variant="link">
              View replies
            </Button>
          </HStack>
        ) : null}
      ...
      {isReply ? <CommentForm parentId={props.parentId || props.comment._id} onCancel={onCancel} /> : null}
      {showSubs ? (
        <Box pl={[4, 0]}>
          <CommentList isLoading={subCommentLoading} parentId={props.comment._id} comments={subComments || []} />
        </Box>
      ) : null}
    </Stack>
  )
...
const Actions = (props: CommentExcerptProps & { editMode: boolean; setEditMode: (val: boolean) => void }) => {
              ...
              <Flex align={'center'} justify={'space-between'}>
                <VStack align="start" spacing={0}>
                  ...
                  <Text
                    color={subColor}
                    fontSize={'sm'}
                    title={DateHelper.getLongDate(new Date(comment.timestamps.createdAt))}
                  >
                    {/* {} */}
                ...
                {isMyComment ? (
                  <Actions editMode={editMode} setEditMode={setEditMode} comment={comment} {...props} />
                ) : null}
              </Flex>
