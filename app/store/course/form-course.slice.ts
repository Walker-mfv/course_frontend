...
export const updateCourseById = createAsyncThunk('courses/updateById', async (data: Partial<ICourse>, thunkApi) => {
...
export const addLecture = createAsyncThunk(
  ...
  async (
    ...
    const state = thunkAPI.getState() as RootState
    const val = state.formCourse.curriculum![payload.sectionIndex].units[payload.unitIndex].lecture
...
export const updateLecture = createAsyncThunk(
  ...
  async (payload: { unitAddress: IUnitAddress; data: Partial<ILecture> }, thunkAPI): Promise<ILecture> => {
    const state = thunkAPI.getState() as RootState
...
export const updateQuiz = createAsyncThunk(
  'courses/updateQuiz',
  async (payload: { unitAddress: IUnitAddress; data: Partial<IQuiz> }, thunkAPI): Promise<IQuiz> => {
    const state = thunkAPI.getState() as RootState
    const [sectionIdx, unitIdx] = selectSectionNUnitIndex(
      payload.unitAddress.sectionId,
      payload.unitAddress.unitId
...
export const deleteQuizQuestion = createAsyncThunk(
  ...
  async (payload: { unitAddress: IUnitAddress; id: string; questionId: string }): Promise<IQuestion> => {
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(updateCourseSection.fulfilled, (state, action) => {
        ...
        let sectionIdx = state.curriculum!.findIndex((item) => item._id == action.meta.arg.sectionId)
        if (sectionIdx > -1) {
      ...
      .addCase(updateCourseLectureVideo.fulfilled, (state, action) => {
        ...
        const [sectionIdx, unitIdx] = getSectionNUnitIdx(sectionId, unitId)(state)
        const item = state.curriculum![sectionIdx].units[unitIdx].lecture
        ;(state.curriculum![sectionIdx].units[unitIdx].lecture as ILecture).video = action.payload
      })
      ...
      .addCase(deleteUnit.fulfilled, (state, action) => {
        const { sectionId, unitId } = action.meta.arg
        const sectionIdx = getSectionIdx(sectionId)(state)
        state.curriculum![sectionIdx].units = state.curriculum![sectionIdx].units.filter((item) => item._id != unitId)
      })
      ...
      .addCase(moveUnitToSection.pending, (state, action) => {
        ...
        if (state.curriculum) {
          ...
          const fromSectionIdx = state.curriculum.findIndex((item) => item._id == unitAddress.sectionId)
          const fromUnitIdx = state.curriculum[fromSectionIdx].units.findIndex((item) => item._id == unitAddress.id)
      ...
      .addCase(swapCourseUnit.pending, (state, action) => {
        ...
        if (state.curriculum) {
          ...
          if (parentAIdx > -1 && parentBIdx > -1) {
            const aIdx = state.curriculum[parentAIdx].units.findIndex((item) => item._id == aId)
            const bIdx = state.curriculum[parentBIdx].units.findIndex((item) => item._id == bId)
            if (aIdx > -1 && bIdx > -1 && (aIdx != bIdx || parentAIdx != parentBIdx)) {
              ;[state.curriculum[parentAIdx].units[aIdx], state.curriculum[parentBIdx].units[bIdx]] = [
      ...
      .addCase(swapCourseSection.pending, (state, action) => {
        ...
        if (state.curriculum) {
          ...
          if (aIdx > -1 && bIdx > -1 && aIdx != bIdx) {
            ;[state.curriculum[aIdx], state.curriculum[bIdx]] = [state.curriculum[bIdx], state.curriculum[aIdx]]
          }
        }
      })
  },
...
const getUnitIdxWithSectionIdx = (sectionIdx: number, unitId: string) => (state: FormCourseSlice) => {
...
export const selectFormCourseObjectives = (state: RootState) => state.formCourse.course?.details.objectives
export const selectFormCourseRequirements = (state: RootState) => state.formCourse.course?.details.requirements
export const selectFormCourseSuitableLearners = (state: RootState) => state.formCourse.course?.details.suitableLearner
// CURRICULUMNk
...
export const selectFormCourseSectionExpandedIndexes = (state: RootState) => state.formCourse.sectionExpandedIndexes
...
export const selectFormCourseSectionById = (id: string) => (state: RootState) =>
  state.formCourse.curriculum ? state.formCourse.curriculum.find((item) => item._id == id) : undefined

export const selectFormCourseUnitByIds = (sectionId: string, unitId: string) => (state: RootState) => {
...
export const selectFormCourseLectureByIndexes = (sectionIdx: number, unitIdx: number) => (state: RootState) => {
...
export const selectFormCourseUnitByIndexes = (sectionIdx: number, unitIdx: number) => (state: RootState) => {
  if (state.formCourse.curriculum) {
