...
// THUNKS
export const xAddCourseToCart = createAsyncThunk('cart/add-course-to-cart', async (item: ICourse, thunkApi) => {
  thunkApi.dispatch(addCourseToCart(item))
  const data = await apiAddCourseToCart(item._id)
  return data
})
export const xRemoveCourseInCart = createAsyncThunk('cart/delete-course-in-cart', async (item: ICourse, thunkApi) => {
  ...
  reducers: {
    ...
    removeCourseInCart: (state, action: PayloadAction<ICourse>) => {
      if (state.data?.courses) {
        state.data.courses = state.data.courses.filter((item) => item._id != action.payload._id)
        state.data && LocalStorageHelper.setCart(state.data)
      }
    },
  },
